insert into DISH (id, description, price, type) values(1,'MAGAARita',45,'Pizza');
insert into DISH (id, description, price, type) values(2,'Meat Eater',65,'Pizza');
insert into DISH (id, description, price, type) values(3,'Vivaldi',55,'Pizza');
insert into DISH (id, description, price, type) values(4,'Nomadic',60,'Pizza');
insert into DISH (id, description, price, type) values(5,'Pommes Frittes, Lille',20,'Fast-food');
insert into DISH (id, description, price, type) values(6,'Pommes Frittes, Medium',25,'Fast-food');
insert into DISH (id, description, price, type) values(7,'Pommes Frittes, Stor',30,'Fast-food');


insert into DRINK (id, description, price) values (1, 'Cola 25cl.',17)
insert into DRINK (id, description, price) values (2, 'Cola 33cl.',20)
insert into DRINK (id, description, price) values (3, 'Cola 0.5l.',25)
insert into DRINK (id, description, price) values (4, 'Cola 1.5l.',35)

insert into DRINK (id, description, price) values (5, 'Carlsberg 33cl.',22)
insert into DRINK (id, description, price) values (6, 'Rødvin 0.7l.',92)
insert into DRINK (id, description, price) values (7, 'Hvidvin 0.7l.',87)

insert into SIDE (id, description, price) values (1, 'Ketchup', 5)
insert into SIDE (id, description, price) values (2, 'Bernaise', 5)
insert into SIDE (id, description, price) values (3, 'Hvidløgs-dressing', 5)
insert into SIDE (id, description, price) values (4, 'Hvidløgsbrød', 22)
insert into SIDE (id, description, price) values (5, 'Hvidløgsbrød med ost', 27)

insert into USER (id, username, password) values (1, 'admin', '$2y$12$JJEygwWasJtD6ICwauE.vuDKdQRXBFHf0aNp89LDqiRXhht1omjrq')





